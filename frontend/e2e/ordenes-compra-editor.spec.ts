...
async function mockCatalogos(page: Page): Promise<void> {
  ...
  await page.route('**/solicitudes-compra**', route => route.fulfill({
    status: 200,
    contentType: 'application/json',
    body: JSON.stringify({ success: true, message: '', errors: [], data: { items: [], page: 1, pageSize: 100, totalCount: 0, totalPages: 0 } })
  }));
  await page.route('**/productos?**', route => route.fulfill({
    status: 200,
    contentType: 'application/json',
    body: JSON.stringify({ success: true, message: '', errors: [], data: { items: [{ id: 21, nombre: 'Producto Uno' }], page: 1, pageSize: 100, totalCount: 1, totalPages: 1 } })
  }));
  await page.route('**/productos/21/variantes**', route => route.fulfill({
    status: 200,
    contentType: 'application/json',
...
test.describe('OrdenCompra editor - rutas, RBAC e idempotencia', () => {
  ...
  test('mantiene Idempotency-Key estable al reintentar una creación rechazada', async ({ page }) => {
    ...
    await page.route('**/ordenes-compra', async route => {
      ...
      if (intento === 1) {
        return route.fulfill({
          status: 200,
          contentType: 'application/json',
          body: JSON.stringify({ success: false, message: 'Conflicto causal simulado', errors: [], data: null })
        });
      }
      return route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({ success: true, message: '', errors: [], data: { id: 77 } })
      });
    });
