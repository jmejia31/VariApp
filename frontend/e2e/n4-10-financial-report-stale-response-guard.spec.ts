...
function resumen(usuariosActivos: number) {
  return { data: { usuariosTotales: usuariosActivos + 3, usuariosActivos, usuariosPrivilegiados: 1, rolesTotales: 2, rolesActivos: 2, rolesSinPermisos: 0, permisosCatalogados: 10, eventosAuditoria: 4, eventosExitosos: 4, eventosRechazados: 0, eventosConError: 0, actividadPorModulo: [], alertas: [] } };
}

const auditoria = { data: { total: 0, exitosos: 0, rechazados: 0, conError: 0, usuariosUnicos: 0, porModulo: [] } };
  ...
  test('una respuesta lenta de un rango anterior no sobrescribe la selección más reciente', async ({ page }) => {
    ...
    await page.route('**/reportes-administrativos/**', async route => {
      ...
      if (path.endsWith('/auditoria-resumen')) {
        ...
        await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify(auditoria) }); return;
      }
      if (path.endsWith('/resumen')) {
        if (desde === '2026-01-01') {
          await new Promise(resolve => setTimeout(resolve, 350));
          await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify(resumen(101)) }); return;
        }
        const value = desde === '2026-02-01' ? 202 : 50;
        await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify(resumen(value)) }); return;
      }
    ...
    const firstRequest = page.waitForRequest(request => {
      ...
      return url.pathname.endsWith('/reportes-administrativos/resumen') && url.searchParams.get('desde') === '2026-01-01';
    });
