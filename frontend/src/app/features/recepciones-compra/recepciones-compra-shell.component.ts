...
export class RecepcionesCompraShellComponent implements OnInit {
  ...
  limpiarFiltros(): void { this.ordenCompraId = null; this.estado = ''; this.desde = ''; this.hasta = ''; this.page = 1; this.cargar(); }
  cambiarPagina(event: PageEvent): void { this.page = event.pageIndex + 1; this.pageSize = event.pageSize; this.cargar(); }
  nuevaRecepcion(): void { void this.router.navigate(['/recepciones-compra/nueva']); }
  verDetalle(id: number): void { void this.router.navigate(['/recepciones-compra', id]); }
