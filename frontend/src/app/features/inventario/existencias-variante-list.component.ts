    ...
    <section class="page" aria-labelledby="existencias-title">
      ...
      <form class="filters" (ngSubmit)="aplicarFiltros()" aria-label="Filtros de existencias">
        <mat-form-field appearance="outline">
          <mat-label>Variante ID</mat-label>
          <input matInput type="number" min="1" name="productoVarianteId" [(ngModel)]="productoVarianteId" />
        </mat-form-field>
        <mat-form-field appearance="outline">
          <mat-label>Almacén ID</mat-label>
          <input matInput type="number" min="1" name="almacenId" [(ngModel)]="almacenId" />
        </mat-form-field>
        <mat-form-field appearance="outline">
          <mat-label>Ubicación ID</mat-label>
          <input matInput type="number" min="1" name="ubicacionAlmacenId" [(ngModel)]="ubicacionAlmacenId" />
        </mat-form-field>
        <mat-form-field appearance="outline">
          <mat-label>Estado de stock</mat-label>
          <mat-select name="estadoStock" [(ngModel)]="estadoStock">
            <mat-option value="">Todos</mat-option>
            <mat-option value="bajo">Bajo mínimo</mat-option>
      ...
      <div class="loading" *ngIf="loading()" aria-live="polite">
        <mat-spinner diameter="36"></mat-spinner><span>Cargando existencias…</span>
      </div>

      <ng-container *ngIf="!loading() && !error()">
        <div class="empty" *ngIf="existencias().length === 0">
          <mat-icon>inventory_2</mat-icon>
          <h2>No hay existencias para los filtros seleccionados</h2>
          <p>Modifica los filtros o registra una existencia para la combinación variante/almacén/ubicación.</p>
        </div>
        ...
        <div class="table-shell" *ngIf="existencias().length > 0">
          <table>
            ...
            <thead>
              <tr>
                <th>Producto / variante</th><th>Almacén / ubicación</th><th>Físico</th><th>Reservado</th>
                <th>Disponible</th><th>Tránsito</th><th>Mín.</th><th>Máx.</th><th>Estado</th>
              </tr>
            </thead>
            <tbody>
              <tr *ngFor="let item of existencias(); trackBy: trackById">
                ...
                <td data-label="Físico">{{ item.stockFisico }}</td>
                <td data-label="Reservado">{{ item.stockReservado }}</td>
                <td data-label="Disponible"><strong>{{ item.stockDisponible }}</strong></td>
                <td data-label="Tránsito">{{ item.stockTransito }}</td>
                <td data-label="Mín.">{{ item.stockMinimo }}</td>
                <td data-label="Máx.">{{ item.stockMaximo ?? '—' }}</td>
                <td data-label="Estado">
                  <span class="status agotado" *ngIf="item.estaAgotada">Agotado</span>
                  <span class="status bajo" *ngIf="!item.estaAgotada && item.tieneStockBajo">Bajo mínimo</span>
                  <span class="status ok" *ngIf="!item.estaAgotada && !item.tieneStockBajo">Disponible</span>
                </td>
              </tr>
            </tbody>
          </table>
          <mat-paginator
            [length]="totalCount()" [pageIndex]="page - 1" [pageSize]="pageSize"
            [pageSizeOptions]="[10, 25, 50, 100]" showFirstLastButtons
            (page)="onPageChange($event)" aria-label="Paginación de existencias">
          </mat-paginator>
        </div>
      </ng-container>
    </section>
...
export class ExistenciasVarianteListComponent implements OnInit {
  ...
  constructor(
    private readonly service: ExistenciaVarianteService,
  ...
  cargar(): void {
    ...
    this.error.set('');
    const filtro: ExistenciaVarianteFiltro = {
      page: this.page,
      pageSize: this.pageSize,
      productoVarianteId: this.positivo(this.productoVarianteId),
      almacenId: this.positivo(this.almacenId),
      ubicacionAlmacenId: this.positivo(this.ubicacionAlmacenId),
      stockBajo: this.estadoStock === 'bajo' ? true : undefined,
      agotada: this.estadoStock === 'agotado' ? true : undefined,
    ...
    this.service.getPaged(filtro).pipe(finalize(() => this.loading.set(false))).subscribe({
      next: response => {
        if (!response.success) {
          ...
          this.error.set(response.message || 'No fue posible cargar las existencias.');
          return;
        }
        this.existencias.set(response.data.items);
      ...
      error: err => {
        this.existencias.set([]);
        this.totalCount.set(0);
        this.error.set(err?.error?.message || err?.error?.title || 'No fue posible cargar las existencias.');
      }
    });
  }
