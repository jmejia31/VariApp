...
export class ReservasInventarioListComponent implements OnInit {
  ...
  cargar(): void { this.loading = true; this.error = ''; const filtro: ReservaInventarioFiltro = { page: this.page, pageSize: this.pageSize, busqueda: this.busqueda.trim() || undefined, estado: this.estado ?? undefined, ventaId: this.ventaId || undefined, almacenId: this.almacenId || undefined }; this.service.getPaged(filtro).pipe(finalize(() => this.loading = false)).subscribe({ next: response => { if (!response.success) { this.error = response.message || 'No se pudieron cargar las reservas.'; return; } this.items = response.data.items; this.totalCount = response.data.totalCount; }, error: () => this.error = 'No se pudieron cargar las reservas.' }); }
