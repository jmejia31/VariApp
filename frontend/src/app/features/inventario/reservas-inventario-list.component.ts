    ...
    <section class="page" aria-labelledby="reservas-title">
      <header class="header">
        <div>
          ...
          <h1 id="reservas-title">Reservas de inventario</h1>
          <p>Controla stock reservado y disponible sin permitir sobreventa.</p>
        </div>
      ...
      <form class="filters" (ngSubmit)="aplicarFiltros()">
        ...
        <mat-form-field appearance="outline"><mat-label>Almacén</mat-label><input matInput type="number" min="1" name="almacenId" [(ngModel)]="almacenId" /></mat-form-field>
      ...
      <div *ngIf="!loading && !error && items.length" class="table-wrap">
        <table>
          <thead><tr><th>Número</th><th>Estado</th><th>Venta</th><th>Expira</th><th>Detalle</th><th>Acciones</th></tr></thead>
          <tbody><tr *ngFor="let item of items">
            <td><strong>{{ item.numero }}</strong><small>{{ item.fechaCreacion | date:'short' }}</small></td>
            <td><span class="badge">{{ item.estado }}</span></td>
            <td>{{ item.ventaId ? ('#' + item.ventaId) : '—' }}</td>
            <td>{{ item.fechaExpiracion ? (item.fechaExpiracion | date:'short') : 'Sin expiración' }}</td>
            <td>{{ item.detalles.length }} línea(s)<small>{{ totalReservado(item) }} unidades reservadas</small></td>
            <td class="actions"><button mat-icon-button type="button" aria-label="Ver reserva" (click)="ver(item.id)"><mat-icon>visibility</mat-icon></button><button *ngIf="puedeEditar && item.estado === 'Borrador'" mat-icon-button type="button" aria-label="Editar reserva" (click)="editar(item.id)"><mat-icon>edit</mat-icon></button></td>
          </tr></tbody>
        </table>
      </div>
...
export class ReservasInventarioListComponent implements OnInit {
  ...
  constructor(private readonly service: ReservaInventarioService, private readonly router: Router, private readonly permisos: PermisosRuntimeService) {}
  ...
  cargar(): void {
    ...
    const filtro: ReservaInventarioFiltro = { page: this.page, pageSize: this.pageSize, busqueda: this.busqueda.trim() || undefined, estado: this.estado ?? undefined, ventaId: this.ventaId || undefined, almacenId: this.almacenId || undefined };
    this.service.getPaged(filtro).pipe(finalize(() => this.loading = false)).subscribe({
      next: response => { if (!response.success) { this.error = response.message || 'No se pudieron cargar las reservas.'; return; } this.items = response.data.items; this.totalCount = response.data.totalCount; },
      error: () => this.error = 'No se pudieron cargar las reservas.'
    });
  }
  aplicarFiltros(): void { this.page = 1; this.cargar(); }
  limpiar(): void { this.busqueda = ''; this.estado = null; this.ventaId = null; this.almacenId = null; this.page = 1; this.cargar(); }
