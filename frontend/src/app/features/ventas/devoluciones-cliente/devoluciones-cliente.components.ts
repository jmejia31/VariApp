...
@Component({
  ...
  @if(esBorrador(d.estado)){<div class="actions">@if(puedeConfirmar()){<button mat-flat-button type="button" (click)="confirmar()" [disabled]="acting()" data-testid="confirmar-devolucion-cliente"><mat-icon>check_circle</mat-icon>Confirmar</button>}@if(puedeAnular()){<mat-form-field appearance="outline"><mat-label>Motivo de anulación</mat-label><input matInput [value]="motivo" (input)="motivo=$any($event.target).value"></mat-form-field><button mat-stroked-button type="button" (click)="anular()" [disabled]="acting()||!motivo.trim()"><mat-icon>cancel</mat-icon>Anular</button>}</div>}
...
export class DevolucionClienteDetailComponent implements OnInit {
  ...
  confirmar():void{const d=this.item();if(!d||!this.esBorrador(d.estado))return;this.acting.set(true);this.service.confirmar(d.id).pipe(finalize(()=>this.acting.set(false))).subscribe({next:r=>this.item.set(r.data??d),error:()=>this.error.set('No fue posible confirmar la devolución.')});}
  anular():void{const d=this.item();if(!d||!this.motivo.trim())return;this.acting.set(true);this.service.anular(d.id,this.motivo).pipe(finalize(()=>this.acting.set(false))).subscribe({next:r=>this.item.set(r.data??d),error:()=>this.error.set('No fue posible anular la devolución.')});}
  esBorrador(v:EstadoDevolucionCliente):boolean{return v===1||v==='Borrador';} estadoNombre(v:EstadoDevolucionCliente):string{return v===1?'Borrador':v===2?'Confirmada':v===3?'Anulada':String(v);} resolucionNombre(v:TipoResolucionDevolucionCliente):string{return v===1||v==='Reintegro'?'Reintegro':v===2||v==='Cambio'?'Cambio':'Crédito a favor';}
}
