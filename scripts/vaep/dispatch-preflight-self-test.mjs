...
const changedFiles = execFileSync('git', ['diff', '--name-only', `${previous}..${head}`], { cwd: root, encoding: 'utf8' }).split(/\r?\n/).filter(Boolean);
if (changedFiles.length === 0) {
  console.error('FIXTURE_INVALID: no files changed between primaryBaseHead and HEAD; cannot prove stale conflict.');
  process.exit(1);
}
const conflictingFile = changedFiles[0];
const refreshableScope = `__vaep_refreshable__/${conflictingFile}`;
...
const cases = [
  ...
  ['stale but refreshable', run({ ...common, dispatchId: 'SELFTEST-N46-ADMISSION-02', primaryBaseHead: previous, fileScopeHint: [refreshableScope] }).outcome === 'REFRESHABLE'],
  ['stale conflicting', run({ ...common, dispatchId: 'SELFTEST-N46-ADMISSION-03', primaryBaseHead: previous, fileScopeHint: [conflictingFile] }).outcome === 'FAIL_CLOSED']
];
