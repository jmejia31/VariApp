-- ERP-N0.2 — validación posterior al retiro de CatalogosProducto
-- Criterio de salida: la última fila (ErroresN02) debe ser 0.

SELECT 'tabla_legacy_presente' AS Regla, COUNT(*) AS Total
...
UNION ALL
SELECT 'modelo_marca_huerfana', COUNT(*)
FROM Modelos mo LEFT JOIN Marcas ma ON ma.Id = mo.MarcaId
WHERE ma.Id IS NULL
UNION ALL
...
SELECT
    (SELECT COUNT(*) FROM information_schema.tables WHERE table_schema=DATABASE() AND table_name='CatalogosProducto')
  ...
  + (SELECT COUNT(*) FROM Modelos mo LEFT JOIN Marcas ma ON ma.Id=mo.MarcaId WHERE ma.Id IS NULL)
