...
public sealed class EvaluacionProveedorConfiguration : IEntityTypeConfiguration<EvaluacionProveedor>
{
    public void Configure(EntityTypeBuilder<EvaluacionProveedor> builder)
    {
        builder.ToTable("EvaluacionesProveedor", table =>
        {
            ...
            table.HasCheckConstraint("CK_EvaluacionesProveedor_UnidadMedida_NoVacia", "CHAR_LENGTH(TRIM(`UnidadMedida`)) > 0");
            ...
            table.HasCheckConstraint("CK_EvaluacionesProveedor_PorcentajeDanado_Rango", "`PorcentajeDanado` >= 0 AND `PorcentajeDanado` <= 100");
        });
        ...
        builder.Property(x => x.PorcentajeDanado).HasPrecision(18, 4);
