...
public sealed class PreparacionPedidoVentaConfiguration : IEntityTypeConfiguration<PreparacionPedidoVenta>
{
    public void Configure(EntityTypeBuilder<PreparacionPedidoVenta> builder)
    {
        ...
        builder.HasIndex(p => p.PedidoVentaId)
            .IsUnique()
            .HasDatabaseName("UX_PreparacionesPedidoVenta_PedidoVentaId");
        builder.HasIndex(p => p.ReservaInventarioId)
            .IsUnique()
        ...
        builder.HasOne(p => p.PedidoVenta)
            .WithMany()
            .HasForeignKey(p => p.PedidoVentaId)
            .OnDelete(DeleteBehavior.Restrict);

        builder.HasOne(p => p.ReservaInventario)
            .WithMany()
            .HasForeignKey(p => p.ReservaInventarioId)
            .OnDelete(DeleteBehavior.Restrict);

        builder.HasMany(p => p.Detalles)
            .WithOne(d => d.PreparacionPedidoVenta)
            .HasForeignKey(d => d.PreparacionPedidoVentaId)
...
public sealed class PreparacionPedidoVentaDetalleConfiguration : IEntityTypeConfiguration<PreparacionPedidoVentaDetalle>
{
    public void Configure(EntityTypeBuilder<PreparacionPedidoVentaDetalle> builder)
    {
        ...
        builder.Property(p => p.ProductoSkuSnapshot).HasMaxLength(100);
        ...
        builder.HasIndex(p => new { p.PreparacionPedidoVentaId, p.ProductoVarianteId, p.AlmacenId, p.UbicacionAlmacenId })
            .IsUnique()
            .HasDatabaseName("UX_PreparacionPedidoVentaDetalles_ClaveFisica");
        builder.HasIndex(p => p.ProductoVarianteId)
            .HasDatabaseName("IX_PreparacionPedidoVentaDetalles_ProductoVarianteId");
        builder.HasIndex(p => p.AlmacenId)
        ...
        builder.HasOne<UbicacionAlmacen>()
            .WithMany()
            .HasForeignKey(p => p.UbicacionAlmacenId)
            .OnDelete(DeleteBehavior.Restrict);
    }
}
