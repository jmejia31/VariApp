...
public class FacturaRepository : IFacturaRepository
{
    ...
    public async Task<CatalogoMetodoPago?> GetMetodoPagoPorCodigoONombreAsync(string valor){var n=valor.Trim().ToUpper();return await _context.Set<CatalogoMetodoPago>().AsNoTracking().FirstOrDefaultAsync(m=>m.Activo&&!m.Eliminado&&(m.Codigo.ToUpper()==n||m.Nombre.ToUpper()==n));}
    public Task<CatalogoBanco?> GetBancoActivoPorIdAsync(int id)=>_context.Set<CatalogoBanco>().AsNoTracking().FirstOrDefaultAsync(b=>b.Id==id&&b.Activo&&!b.Eliminado);
