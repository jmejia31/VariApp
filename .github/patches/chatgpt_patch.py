...
service_path = "backend/src/Application/Services/ProductoVarianteService.cs"
write(service_path, r'''using InventoryApp.Application.DTOs;
...
# Las entidades de producto cargadas por repositorio tienen tracking. Evitar Update()
# impide marcar como modificada una cantidad obsoleta durante cambios de metadatos.
producto_path = "backend/src/Application/Services/ProductoService.cs"
producto = read(producto_path)
producto = producto.replace("        _repository.Update(producto);\n", "")
write(producto_path, producto)

# Adaptar las pruebas unitarias al contrato transaccional y a los métodos FOR UPDATE.
test_path = "backend/tests/InventoryApp.Tests/ProductoVarianteServiceTests.cs"
test = read(test_path)
test = test.replace(
...
print("Mantenimiento de variantes serializado y marcado global eliminado.")
