...
\t'}"
    if [[ "$status" != "A" ]]; then
      printf 'VAEP MASTER INVALID_REDISPATCH: dispatch commits are append-only; status=%s path=%s.\n' "$status" "$path" >&2
      return 65
    fi
    if [[ ! "$path" =~ ^vaep/jules(-b|-c|-d)?/dispatch/[^/]+\.json$ ]]; then
      printf 'VAEP MASTER transport invariant failed: non-dispatch file in atomic batch: %s.\n' "$path" >&2
      return 22
    fi
  done
  VALIDATED_MANIFEST="${manifests[0]}"
