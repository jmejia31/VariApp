...
manifest_transport_action() {
  local added="${1:?added count required}"
  local non_added="${2:-0}"
  if (( non_added > 0 )); then
    printf 'INVALID_REDISPATCH\n'
    return 65
  fi
  case "$added" in
    0) printf 'INVALID_TRIGGER\n'; return 64 ;;
    1) printf 'ADMIT\n'; return 0 ;;
    *) printf 'FAIL_CLOSED\n'; return 21 ;;
  esac
}
...
write_timeout_result() {
  ...
  local result_state="${11:-JULES_LANE_BUDGET_EXCEEDED}"
...
if [[ "${1:-}" == "--self-test" ]]; then
  ...
  bash "$PARSER" --self-test >/dev/null
  set +e
  invalid_action="$(manifest_transport_action 0 0)"; invalid_rc=$?
  redispatch_action="$(manifest_transport_action 0 1)"; redispatch_rc=$?
  admit_action="$(manifest_transport_action 1 0)"; admit_rc=$?
  multi_action="$(manifest_transport_action 2 0)"; multi_rc=$?
  set -e
  [[ "$invalid_action" == "INVALID_TRIGGER" && "$invalid_rc" -eq 64 ]]
  [[ "$redispatch_action" == "INVALID_REDISPATCH" && "$redispatch_rc" -eq 65 ]]
  [[ "$admit_action" == "ADMIT" && "$admit_rc" -eq 0 ]]
...
validate_dispatch_transport "$DISPATCH_SHA"
manifest="$VALIDATED_MANIFEST"

if [[ "${1:-}" == "--transport-preflight" ]]; then
  printf 'VAEP TRANSPORT_PREFLIGHT=PASS dispatch_sha=%s worker=%s manifest=%s\n' "$DISPATCH_SHA" "$WORKER_ID" "$manifest"
  exit 0
fi
admission_rc=0
...
dispatch_parent="$(git rev-parse "${DISPATCH_SHA}^")"
...
if [[ -f "$runtime_state" ]] && jq -e 'type == "object"' "$runtime_state" >/dev/null 2>&1; then
  ...
  runtime_phase="$(jq -r '.phase // empty' "$runtime_state")"
fi
runtime_phase="${runtime_phase:-}"
timeout_result_state="JULES_LANE_BUDGET_EXCEEDED"
if [[ "$rc" -eq 125 || "$runtime_phase" == "STALL_NO_PROGRESS" ]]; then
...
write_timeout_result "$result_dir/result.json" "$dispatch_id" "$task_id" "$task_attempt" "$session_name" "$started_at" "$timed_out_at" "$safe_stop_action" "$before_state" "$after_state" "$timeout_result_state"
...
printf -v body '%s\n\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n\n%s\n' \
  "VAEP $WORKER_LABEL MASTER — $timeout_result_state; session superseded." \
...
printf 'VAEP MASTER %s for %s (%s). Ownership revoked, session superseded and lane released for controller failover.\n' "$timeout_result_state" "$dispatch_id" "$task_id" >&2
exit "$rc"
