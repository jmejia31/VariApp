#!/usr/bin/env bash
set -euo pipefail
readonly VAEP_JULES_CLARIFICATION_GUARD="v3.27"
...
tmp="$RUNNER_TEMP/vaep-jules-v327-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$tmp"
trap 'rm -rf "$tmp"' EXIT
patched_worker="$tmp/vaep-jules-worker-v327.sh"
patched_guard="$tmp/vaep-jules-throughput-guard-v327.sh"
...
if text.count(old)!=1: raise SystemExit('v3.27 INNER_WORKER invariant failed')
guard.write_text(text.replace(old,f'readonly INNER_WORKER="{worker}"'),encoding='utf-8')
PY
...
if [[ "${1:-}" == "--static-self-test" ]]; then
  ...
  printf '{"status":"ok","clarificationGuard":"%s","deterministicRepoFallback":true}\n' "$VAEP_JULES_CLARIFICATION_GUARD"
  exit 0
fi
