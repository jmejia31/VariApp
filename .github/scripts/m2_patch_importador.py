...
PATH = Path("backend/src/Infrastructure/Services/CargaMasivaService.cs")
text = PATH.read_text(encoding="utf-8")

VALIDAR_START = "    private async Task ValidarVariantesAsync("
VALIDAR_END = "\n    private async Task<(int Creados, int Actualizados)> AplicarClientesAsync("
APLICAR_START = "    private async Task<(int Creados, int Actualizados)> AplicarVariantesAsync("
APLICAR_END = "\n    private static CargaMasivaDto MapResumenExpression("

validar = '''    private async Task ValidarVariantesAsync(List<CargaMasivaFilaDto> filas, List<CargaMasivaErrorDto> errores, CancellationToken ct)
    {
...
aplicar = '''    private async Task<(int Creados, int Actualizados)> AplicarVariantesAsync(
        int cargaId,
...
def replace_between(source: str, start: str, end: str, replacement: str) -> str:
    a = source.find(start)
    if a < 0:
        raise SystemExit(f"No se encontró marcador inicial: {start}")
    b = source.find(end, a)
    if b < 0:
        raise SystemExit(f"No se encontró marcador final: {end}")
    if source.find(start, a + 1) >= 0:
        raise SystemExit(f"Marcador inicial duplicado: {start}")
    return source[:a] + replacement.rstrip() + source[b:]

text = replace_between(text, VALIDAR_START, VALIDAR_END, validar)
text = replace_between(text, APLICAR_START, APLICAR_END, aplicar)
