...
readonly ALLOWED_KEYS=(
  "EXECUTION_MODEL"
  "DIRECT_EXECUTION_DEFAULT"
  "JULES_REQUIRED_FOR_PROGRESS"
  "JULES_OFFLOAD_ONLY_IF_CRITICAL_PATH_GAIN"
  "JULES_MINIMUM_UTILIZATION_TARGET"
  "OFFLOAD_REQUIRES_NONOVERLAP"
  "OFFLOAD_NEVER_BLOCKS_CURRENT_PARENT"
  "PRIMARY_DIRECT_BUILD_CLOSE"
  "SUPERVISOR_VERIFY_RECOVER_SECONDARY_BUILD"
  "TASK_SCOPE_LEASE_REQUIRED"
  "TASK_SCOPE_LEASE_SINGLE_WRITER"
  "TASK_SCOPE_LEASE_TTL_MINUTES"
  "STALE_LEASE_TAKEOVER_AFTER_MINUTES"
  "LEASE_FRESH_REQUIRES_LIVE_MATERIAL_PROGRESS"
  "ENDED_INVOCATION_RELEASES_LEASE_IMMEDIATELY"
  "DIRECT_NEXT_SAFE_PREARM_REQUIRED"
  "PARENT_CLOSE_SLA_ROLLING_60M"
  "PARENT_CLOSE_SLA_ROLLING_24H"
  ...
  "CAUSAL_HOLD_SCOPE"
  "CAUSAL_GATE_MATRIX_REQUIRED"
  "UNRELATED_WORKFLOW_BLOCKING_PROHIBITED"
  "REVIEW_BEFORE_EXPENSIVE_CI"
  "CLOSURE_CANDIDATE_HEAD_FREEZE"
  "LOGICAL_CLOSER_CONTINUITY"
  "SPLIT_BEFORE_WRITE_OVER_20M"
  "CYCLE_TIME_TELEMETRY_REQUIRED"
  "NO_MANIFEST_DURING_HEAD_FREEZE_CAUSAL"
  "PREARM_BEFORE_CAUSAL_CI"
  "VAEP_CHECKPOINTS"
  "VAEP_SUPERVISOR_CHECKPOINTS"
  "VAEP_ALL_ACTIVE_SLOTS"
  "JULES_ACTIVE_WORKERS"
  "JULES_MAX_ATTEMPTS"
  "JULES_REWORK_MAX"
  "JULES_LANE_BUDGET_SECONDS"
  "DEFECT_RECOVERY_FIRST"
  "FIRST_DETECTOR_OWNS_RECOVERY"
  "NO_REJECT_QUEUE"
  "CROSS_LANE_R2_ALLOWED"
  "SECOND_ATTEMPT_CONTROLLER_TAKEOVER_REQUIRED"
  "RECOVERY_MUST_RESOLVE_SAME_RUN"
  "RECOVERY_UNBLOCK_DEPENDENTS_SAME_RUN"
  "PARENT_CLOSE_FIRST"
)
...
is_boolean_key() {
  case "$1" in
    DIRECT_EXECUTION_DEFAULT|JULES_REQUIRED_FOR_PROGRESS|JULES_OFFLOAD_ONLY_IF_CRITICAL_PATH_GAIN|OFFLOAD_REQUIRES_NONOVERLAP|OFFLOAD_NEVER_BLOCKS_CURRENT_PARENT|PRIMARY_DIRECT_BUILD_CLOSE|SUPERVISOR_VERIFY_RECOVER_SECONDARY_BUILD|TASK_SCOPE_LEASE_REQUIRED|TASK_SCOPE_LEASE_SINGLE_WRITER|LEASE_FRESH_REQUIRES_LIVE_MATERIAL_PROGRESS|ENDED_INVOCATION_RELEASES_LEASE_IMMEDIATELY|DIRECT_NEXT_SAFE_PREARM_REQUIRED|CLOSURE_CHAIN_SAME_RUN|GLOBAL_FROZEN_PROHIBITED|CAUSAL_GATE_MATRIX_REQUIRED|UNRELATED_WORKFLOW_BLOCKING_PROHIBITED|REVIEW_BEFORE_EXPENSIVE_CI|CLOSURE_CANDIDATE_HEAD_FREEZE|LOGICAL_CLOSER_CONTINUITY|SPLIT_BEFORE_WRITE_OVER_20M|CYCLE_TIME_TELEMETRY_REQUIRED|NO_MANIFEST_DURING_HEAD_FREEZE_CAUSAL|PREARM_BEFORE_CAUSAL_CI|DEFECT_RECOVERY_FIRST|FIRST_DETECTOR_OWNS_RECOVERY|NO_REJECT_QUEUE|CROSS_LANE_R2_ALLOWED|SECOND_ATTEMPT_CONTROLLER_TAKEOVER_REQUIRED|RECOVERY_MUST_RESOLVE_SAME_RUN|RECOVERY_UNBLOCK_DEPENDENTS_SAME_RUN|PARENT_CLOSE_FIRST) return 0 ;;
    *) return 1 ;;
  esac
}
...
is_string_key() {
  case "$1" in
    EXECUTION_MODEL|VAEP_CHECKPOINTS|VAEP_SUPERVISOR_CHECKPOINTS|VAEP_ALL_ACTIVE_SLOTS|JULES_ACTIVE_WORKERS|GLOBAL_DISPATCH_ADMISSION|CAUSAL_HOLD_SCOPE) return 0 ;;
    *) return 1 ;;
  esac
}
...
parse_policy_block() {
  ...
  while IFS= read -r raw_line || [[ -n "$raw_line" ]]; do
    ...
    case "$key" in
      TASK_SCOPE_LEASE_TTL_MINUTES|STALE_LEASE_TAKEOVER_AFTER_MINUTES|PARENT_CLOSE_SLA_ROLLING_60M|PARENT_CLOSE_SLA_ROLLING_24H|PARENT_MAX_DWELL_MINUTES|PARENT_STALL_NO_PROGRESS_MINUTES|CLOSURE_REVIEW_MAX_LATENCY_MINUTES|CLOSURE_DEBT_TRIGGER_LT|JULES_MAX_ATTEMPTS|JULES_LANE_BUDGET_SECONDS)
        [[ "$val" =~ ^[1-9][0-9]*$ ]] || fail "invalid positive integer value for $key: '$val'" ;;
      JULES_MINIMUM_UTILIZATION_TARGET|JULES_REWORK_MAX|MAX_VOLUNTARY_IDLE)
        [[ "$val" =~ ^(0|[1-9][0-9]*)$ ]] || fail "invalid non-negative integer value for $key: '$val'" ;;
      VAEP_CHECKPOINTS|VAEP_SUPERVISOR_CHECKPOINTS|VAEP_ALL_ACTIVE_SLOTS)
        [[ "$val" =~ ^:[0-5][0-9](,:[0-5][0-9])*$ ]] || fail "invalid checkpoint list for $key: '$val'" ;;
      EXECUTION_MODEL)
        [[ "$val" == "TASKS_FIRST_JULES_ON_DEMAND" ]] || fail "invalid value for $key: '$val'" ;;
      JULES_ACTIVE_WORKERS)
        [[ "$val" == "J1,J2,J3,J4,J5,J6" ]] || fail "invalid value for $key: '$val'" ;;
      GLOBAL_DISPATCH_ADMISSION)
        [[ "$val" == "OPEN_ONLY" ]] || fail "invalid value for $key (must be OPEN_ONLY): '$val'" ;;
      CAUSAL_HOLD_SCOPE)
        [[ "$val" == "TASK_OR_EXECUTION_ONLY" ]] || fail "invalid value for $key (must be TASK_OR_EXECUTION_ONLY): '$val'" ;;
      *)
