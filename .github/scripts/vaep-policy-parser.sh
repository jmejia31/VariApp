...
is_boolean_key() {
  case "$1" in
    DIRECT_EXECUTION_DEFAULT|JULES_REQUIRED_FOR_PROGRESS|JULES_OFFLOAD_ONLY_IF_CRITICAL_PATH_GAIN|OFFLOAD_REQUIRES_NONOVERLAP|OFFLOAD_NEVER_BLOCKS_CURRENT_PARENT|PRIMARY_DIRECT_BUILD_CLOSE|SUPERVISOR_VERIFY_RECOVER_SECONDARY_BUILD|TASK_SCOPE_LEASE_REQUIRED|TASK_SCOPE_LEASE_SINGLE_WRITER|DIRECT_NEXT_SAFE_PREARM_REQUIRED|CLOSURE_CHAIN_SAME_RUN|GLOBAL_FROZEN_PROHIBITED|NO_MANIFEST_DURING_HEAD_FREEZE_CAUSAL|PREARM_BEFORE_CAUSAL_CI|DEFECT_RECOVERY_FIRST|FIRST_DETECTOR_OWNS_RECOVERY|NO_REJECT_QUEUE|CROSS_LANE_R2_ALLOWED|SECOND_ATTEMPT_CONTROLLER_TAKEOVER_REQUIRED|RECOVERY_MUST_RESOLVE_SAME_RUN|RECOVERY_UNBLOCK_DEPENDENTS_SAME_RUN|PARENT_CLOSE_FIRST) return 0 ;;
    *) return 1 ;;
  esac
}
