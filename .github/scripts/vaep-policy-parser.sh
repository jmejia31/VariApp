...
readonly ALLOWED_KEYS=(
  ...
  OWNER_INTERVENTION_GATE_ACTIVE OWNER_INTERVENTION_AUTOMATIONS_PAUSED
  OWNER_PAUSE_OVERRIDES_CANONICAL_LIVENESS OWNER_INTERVENTION_QUEUE_ANCHOR_ROW
  OWNER_INTERVENTION_FIRST_ROW OWNER_INTERVENTION_ENTRY_AFTER
  OWNER_INTERVENTION_ALLOWED_PARENTS OWNER_INTERVENTION_EXIT
  OWNER_INTERVENTION_RESUME_FROM OWNER_INTERVENTION_BYPASS_PROHIBITED
  POST_INTERVENTION_REVALIDATION_REQUIRED POST_INTERVENTION_REVALIDATION_START_ROW
  POST_INTERVENTION_REVALIDATE_EXISTING_LISTO POST_INTERVENTION_REVALIDATION_BASIS
  POST_INTERVENTION_REOPEN_ON_GAP POST_INTERVENTION_HISTORICAL_STATUS_PRESERVED
  POST_INTERVENTION_REVALIDATION_ADMISSION POST_INTERVENTION_REVALIDATION_STATE
  POST_INTERVENTION_BULK_REOPEN_PROHIBITED
)
...
is_string_key(){
  case "$1" in
    EXECUTION_MODEL|VAEP_CHECKPOINTS|VAEP_SUPERVISOR_CHECKPOINTS|VAEP_ALL_ACTIVE_SLOTS|GLOBAL_DISPATCH_ADMISSION|CAUSAL_HOLD_SCOPE|OWNER_INTERVENTION_ENTRY_AFTER|OWNER_INTERVENTION_ALLOWED_PARENTS|OWNER_INTERVENTION_EXIT|OWNER_INTERVENTION_RESUME_FROM|POST_INTERVENTION_REVALIDATION_BASIS|POST_INTERVENTION_REVALIDATION_ADMISSION|POST_INTERVENTION_REVALIDATION_STATE) return 0;;
    *) return 1;;
  esac
}
...
parse_policy_block(){
  ...
  while IFS= read -r raw_line || [[ -n "$raw_line" ]]; do
    ...
    case "$key" in
      ...
      OWNER_INTERVENTION_ENTRY_AFTER|OWNER_INTERVENTION_EXIT|OWNER_INTERVENTION_RESUME_FROM) [[ "$val" =~ ^N8\.[0-9]+\.[A-H]$ ]] || fail "invalid parent/task reference for $key: '$val'";;
      ...
      POST_INTERVENTION_REVALIDATION_ADMISSION) [[ "$val" == JIT_REOPEN_ONE_AT_A_TIME ]] || fail "invalid value for $key: '$val'";;
