...
- Trayectoria de recuperación: los cinco checkpoints activos `:00/:12/:24/:36/:48` garantizan primero únicamente `CURRENT_RUN + NEXT_RUN_RESERVED`. Con esa continuidad satisfecha, `CLOSURE_DEBT_FASTPATH` domina hasta cerrar/promover el CURRENT_PARENT. No existe floor profundo de 20 ni emergencia por UNUSED: el catálogo es pequeño y material, y el controller lo repone sólo con scopes genuinos del parent vigente. El único SLA horario canónico es `PARENT_CLOSE_SLA_ROLLING_60M=3`; ningún checkpoint histórico es operativo.
...
- `JULES_QUEUE_DEPTH_TARGET=2`: objetivo y límite físico por lane = exactamente `1 CURRENT_RUN + como máximo 1 NEXT_RUN_RESERVED`. Nunca crear un tercer run vivo.
...
- `ONE_MANIFEST_ONE_RUN=TRUE`: un manifest se publica en un único commit y el workflow nace exclusivamente del trigger `push:path`. El autorefill NO debe emitir `workflow_dispatch` adicional para la misma reserva.
...
- El autorefill debe crear exactamente un manifest nuevo, con `primaryBaseHead` igual al padre real del commit, respetar `dispatch-admission=OPEN`, no duplicar scopes/dispatches y detenerse ante `HEAD_FREEZE_CAUSAL` real. Una actualización concurrente de HEAD obliga a reintentar contra el nuevo padre, nunca a publicar una base stale.
- El catálogo de autorefill contiene únicamente trabajo material y scope exclusivo del `CURRENT_PARENT`. Profundidad objetivo = `CURRENT + NEXT` (`JULES_QUEUE_DEPTH_TARGET=2`). La regeneración genérica de facetas está PROHIBIDA: si el catálogo se agota, el controller debe crear un scope material realmente nuevo del parent vigente o cerrar/promover el parent; nunca reciclar una identidad completada, renumerar la misma prueba ni fabricar busywork.
...
- Cuando el CURRENT terminaliza y consume la reserva existente, el mismo workflow debe ejecutar AUTOREFILL antes de cerrar y dejar un nuevo run `pending|queued|in_progress` siempre que exista SAFE_WORK y no haya freeze causal. Esperar al siguiente checkpoint teniendo catálogo material disponible es incumplimiento.
