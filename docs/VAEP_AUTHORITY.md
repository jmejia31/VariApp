...
- Trayectoria de recuperación: los cinco checkpoints activos `:00/:12/:24/:36/:48` garantizan primero sólo la continuidad mínima real `CURRENT_RUN + NEXT_RUN_RESERVED`. Con esa continuidad satisfecha, si `ROLLING60<3` o existe parent certificable/review-ready, `CLOSURE_DEBT_FASTPATH` tiene prioridad sobre rellenar el catálogo profundo a 20. Sólo una lane sin CURRENT/NEXT o `UNUSED<=8` puede interrumpir temporalmente el cierre por continuidad/emergencia. El único SLA horario canónico es `PARENT_CLOSE_SLA_ROLLING_60M=3`; ningún checkpoint histórico `:05/:10/:15/:20/:25/:30/:35/:40/:45/:50/:55` es operativo.
...
- El catálogo de autorefill contiene únicamente trabajo material, scope exclusivo y no busywork. Objetivo profundo: 20 entradas UNUSED por lane. Este piso es preventivo y **no puede bloquear cierre padre**: con CURRENT+NEXT ya reales y `ROLLING60<3`, VAEP drena REVIEW/CERT/CLOSE antes de rellenar de 19→20. Si llega a 8 o menos, es `EMERGENCY_BACKLOG_DEBT` y el refill vuelve a prioridad alta. Nunca esperar a cero, nunca reciclar una identidad funcional completada y nunca inventar APIs, producción o evidencia.
