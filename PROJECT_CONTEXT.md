...
- Cloudinary API-key inventory: el cloud `riyrzmob` tiene actualmente solo una key activa `Root` (creada 2026-09-22); no existe aún `solqaryn_dev`. La key Root se reserva para administración/recuperación; DEV debe usar una key dedicada `solqaryn_dev` antes de cerrar Cloudinary.
