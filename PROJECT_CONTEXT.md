...
- Cloudinary API keys canónicas creadas el 2026-09-25 en `riyrzmob`: `Root` para administración/recuperación, `solqaryn_dev` para Render DEV y `solqaryn_prod` reservada para la futura fase PROD. PROD no se conecta todavía. DEV queda pendiente de configurar Render con `solqaryn_dev` y validar un upload real bajo `solqaryn_dev/`.
