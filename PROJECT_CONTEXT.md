...
- Vercel DEV certificado y cerrado el 2026-09-25: team `SOLQARYN` (`team_owJ2SudSPWiEzeiDthSVV063`), único proyecto visible `solqaryn-dev` (`prj_1Anhx5mWyXEBX89lWC24Py6JXe7A`), dominio `solqaryn-dev.vercel.app`, deploys desde `solqaryn/Solqaryn` rama `dev`, último deployment observado `READY`, rutas plataforma/storefront y APIs DEV en HTTP 200, sin runtime errors en las últimas 24h. Evidencia visual del panel confirmó que la sesión que administra el workspace/team usa `solqaryn.platform@outlook.com`. El proyecto legacy `variapp-desarrollo` permanece únicamente como pendiente de retiro desde la cuenta personal antigua; no tocar `varistorehn` PROD en esta fase.
- Evidencia visual adicional del 2026-09-25 en la cuenta Vercel personal legacy (`jmejia31` / workspace `VariApp`) confirma que allí siguen existiendo exactamente dos proyectos visibles: `variapp-desarrollo` y `varistorehn`. Para el cierre DEV se audita y retira únicamente `variapp-desarrollo`; `varistorehn` queda congelado por corresponder a Producción legacy.
- Vercel legacy DEV autorizado para retiro: `variapp-desarrollo` no tiene dominios personalizados (solo el alias automático `variapp-desarrollo.vercel.app`) ni Environment Variables de proyecto. Puede eliminarse manualmente desde la cuenta personal. `varistorehn` PROD no entra en alcance.
- Vercel legacy DEV eliminado manualmente el 2026-09-25: evidencia visual posterior muestra que en el workspace personal `VariApp` ya sólo permanece `varistorehn`. Por tanto `variapp-desarrollo` queda retirado definitivamente; Vercel DEV no depende ya de la cuenta personal.
...
- Cloudinary API keys canónicas creadas el 2026-09-25 en `riyrzmob`: `Root` para administración/recuperación, `solqaryn_dev` para Render DEV y `solqaryn_prod` reservada para la futura fase PROD. Render DEV ya está configurado con `Cloudinary__CloudName=riyrzmob` y `Cloudinary__EnvironmentPrefix=solqaryn_dev`; el redeploy posterior quedó `live` y health 200. Solo falta un upload funcional para certificar que el nuevo asset se crea bajo `solqaryn_dev/`. PROD no se conecta todavía.
