- Retiro DEV personal — estado 2026-09-25: Vercel `variapp-desarrollo` ya fue eliminado; Render canónico sólo expone el workspace `SOLQARYN` y los servicios `solqaryn-api-dev`/reservado PROD, mientras el servicio histórico DEV `solqaryn-api-desarrollo` (ID histórico `srv-d9jblq7avr4c73c74jng`) no es accesible desde el workspace corporativo y su hostname público ya no resuelve. Inventario Aiven read-only run `36193976802` con el token corporativo ve únicamente proyecto `solqaryn`/servicio `solqaryn-mysql`; no ve el proyecto personal legacy `variapp`, por lo que NO se autoriza borrar todavía el servicio Aiven personal ni `defaultdb`/`varistorehn_desarrollo` hasta auditar PROD legacy desde la cuenta antigua. Cloudinary legacy tampoco se borra hasta demostrar cero consumidores PROD. Cloudflare corporativo contiene zona `solqaryn.com` en estado pending y sólo dos TXT `_acme-challenge`; no existen aliases DEV legacy allí. GitHub: `jmejia31` conserva admin por ser Owner secundario/de recuperación ratificado; no se elimina sin nueva decisión explícita.
- Certificación funcional final DEV cerrada el 2026-09-25. GitHub Actions run `36192919335` terminó SUCCESS con: rutas `/login`, `/dashboard`, `/varistorehn`, `/varistorehn/productos` accesibles; branding plataforma SOLQARYN en login/dashboard; identidad pública VariStorehn; categorías y 4 productos públicos; todas las imágenes Cloudinary verificadas HTTP 200 bajo `riyrzmob`; Render `/health/ready` HTTP 200; escritura/lectura efímera en `solqaryn_dev` PASS; cero referencias operativas a `variapp-desarrollo`, al host Aiven personal `variapp-mysql-variapp.c.aivencloud.com` o a `varistorehn_desarrollo` en runtime. Logs Render actuales muestran exclusivamente `solqaryn_dev` en `solqaryn-mysql-solqaryn.h.aivencloud.com`. PROD no fue tocado.
- Cloudflare DEV/DNS auditado el 2026-09-25: el runtime canónico DEV no depende de ningún hostname custom ni zona Cloudflare. Vercel project `solqaryn-dev` expone únicamente dominios `*.vercel.app` (`solqaryn-dev.vercel.app`, `solqaryn-dev-solqaryn.vercel.app`, `solqaryn-dev-git-dev-solqaryn.vercel.app`); Render DEV usa exclusivamente `https://solqaryn-api-dev-fxx8.onrender.com`. El repositorio no contiene configuración Cloudflare ni dominio `solqaryn.com` para DEV. Resultado: Cloudflare es **N/A como dependencia crítica DEV** y no bloquea el retiro de infraestructura personal DEV. La propiedad de la cuenta Cloudflare queda como auditoría separada de plataforma/futuros dominios y de PROD.
- Reconciliación de stock DEV completada el 2026-09-25. Hallazgo causal: la administración mostraba el bridge legacy `ProductoVariante.Cantidad`, mientras la tienda pública usa la autoridad física `ExistenciaVariante.StockDisponible`. Tras la migración de datos, seis variantes de productos activos no tenían fila física, por lo que la tienda fallaba cerrada con stock 0. Se ejecutó backup cifrado + restore drill (run `36188392969`) y luego backfill fail-closed sobre el único almacén operativo activo. Reconciliación run `36190745792`: 6 existencias creadas, stock activo legacy 52 = stock físico 52, 0 variantes activas sin existencia, 0 diferencias. Totales públicos verificados: Cargador 26, Laptop 15, Funda para samsung 1, UAT Producto 001 10. Dos variantes pertenecientes a productos eliminados suman 5 unidades legacy y quedaron deliberadamente fuera del stock activo/storefront; no se reactivaron ni materializaron como stock vendible.
...
- Vercel DEV certificado y cerrado el 2026-09-25: team `SOLQARYN` (`team_owJ2SudSPWiEzeiDthSVV063`), único proyecto visible `solqaryn-dev` (`prj_1Anhx5mWyXEBX89lWC24Py6JXe7A`), dominio `solqaryn-dev.vercel.app`, deploys desde `solqaryn/Solqaryn` rama `dev`, último deployment observado `READY`, rutas plataforma/storefront y APIs DEV en HTTP 200, sin runtime errors en las últimas 24h. Evidencia visual del panel confirmó que la sesión que administra el workspace/team usa `solqaryn.platform@outlook.com`. El proyecto legacy `proyecto Vercel DEV legacy retirado` permanece únicamente como pendiente de retiro desde la cuenta personal antigua; no tocar `varistorehn` PROD en esta fase.
- Evidencia visual adicional del 2026-09-25 en la cuenta Vercel personal legacy (`jmejia31` / workspace `workspace Vercel personal legacy`) confirma que allí siguen existiendo exactamente dos proyectos visibles: `proyecto Vercel DEV legacy retirado` y `varistorehn`. Para el cierre DEV se audita y retira únicamente `proyecto Vercel DEV legacy retirado`; `varistorehn` queda congelado por corresponder a Producción legacy.
- Vercel legacy DEV autorizado para retiro: `proyecto Vercel DEV legacy retirado` no tiene dominios personalizados (solo el alias automático `alias automático Vercel DEV retirado`) ni Environment Variables de proyecto. Puede eliminarse manualmente desde la cuenta personal. `varistorehn` PROD no entra en alcance.
- Vercel legacy DEV eliminado manualmente el 2026-09-25: evidencia visual posterior muestra que en el workspace personal `workspace Vercel personal legacy` ya sólo permanece `varistorehn`. Por tanto `proyecto Vercel DEV legacy retirado` queda retirado definitivamente; Vercel DEV no depende ya de la cuenta personal.
- Cloudinary ownership DEV verificado visualmente el 2026-09-25: sesión/perfil `Solqaryn Platform` con correo `solqaryn.platform@outlook.com`; cuenta con un único Product Environment activo, cloud name `riyrzmob`, ID `7ab9e3e6de660a0b70eb4a5bacf331`. La Media Library del nuevo cloud muestra únicamente contenido de ejemplo, por lo que todavía falta certificar qué API key usa Render DEV y migrar/crear activos DEV bajo `solqaryn_dev/` antes de retirar cualquier Cloudinary legacy.
- Cloudinary API keys canónicas creadas el 2026-09-25 en `riyrzmob`: `Root` para administración/recuperación, `solqaryn_dev` para Render DEV y `solqaryn_prod` reservada para la futura fase PROD. Render DEV está configurado con `Cloudinary__CloudName=riyrzmob` y `Cloudinary__EnvironmentPrefix=solqaryn_dev`; el redeploy quedó `live` y health 200. Upload funcional confirmado desde la aplicación: un asset nuevo fue creado y persistido bajo `solqaryn_dev/inventoryapp/productos/empresas/1/` y la API pública devuelve la URL del cloud `riyrzmob`. Inventario read-only run `36182095589` detectó todavía 13 filas lógicas con referencias al Cloudinary legacy: 10 `ProductoImagenes`, 1 `CompraDocumentos` y 2 fotos de perfil de `Usuarios`. Por tanto Cloudinary DEV nuevo funciona, pero el punto no se cierra hasta migrar esas 13 referencias y dejar cero consumidores del cloud personal. PROD no se conecta todavía.
